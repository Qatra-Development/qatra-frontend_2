...
import { bankApi, bankListAll, type BloodRequest, type BloodUnit } from "../lib/api";
...
const asRow = (request: BloodRequest): RequestRow => ({
  ...
  hospital: request.requesting_institution?.name ?? "—",
  createdBy: request.requesting_institution?.name ?? "—",
  type: request.blood_type,
  units: request.units_required,
  urgency: request.priority === "emergency" ? "طارئ" : request.priority === "urgent" ? "عاجل" : "عادي",
  status: request.status,
  date: request.needed_at ? new Date(request.needed_at).toLocaleString("ar") : "—",
  raw: request,
});
...
export default function LatestRequests({ showHeader = true, toolbar, incomingMode = false, statusOverride, onChanged, onCompleted, onReady }: LatestRequestsProps) {
  ...
  const refreshSelected = async (id: number) => {
    const detail = (await bankApi<BloodRequest>(`/requests/${id}`)).data;
    setSelectedRequest(asRow(detail));
    await load();
    onChanged?.();
    return detail;
  };
  const perform = async (action: string, body?: object) => {
    ...
    setBusy(true);
    try {
      await bankApi<BloodRequest>(`/requests/${selectedRequest.backendId}/${action}`, {
        method: "POST",
        ...(body ? { body: JSON.stringify(body) } : {}),
      });
      const detail = await refreshSelected(selectedRequest.backendId);
      setError("");
      if (action === "complete") {
      ...
      await refreshSelected(selectedRequest.backendId).catch(() => undefined);
    } finally { setBusy(false); }
  };
  const getStatus = (request: RequestRow): RequestStatus =>
    request.raw.status === "completed" ? "completed" :
  ...
  const openDetails = async (request: RequestRow) => {
    ...
    try { await refreshSelected(request.backendId); }
    catch (cause) { setError(cause instanceof Error ? cause.message : "تعذّر تحميل تفاصيل الطلب."); }
  };

  const openUnitsDialog = async () => {
    if (!selectedRequest) return;
    setSelectedUnits([]);
    try {
      const all = await bankListAll<BloodUnit>(`/requests/${selectedRequest.backendId}/eligible-units`);
      setAvailableUnits(all.filter((unit) => unit.blood_type === selectedRequest.type));
      dialogRef.current?.close();
  ...
  const toggleUnit = (id: number) => {
    setSelectedUnits((current) => current.includes(id)
      ...
      : current.length < ((selectedRequest?.units ?? 0) - (selectedRequest?.raw.units_reserved ?? 0)) ? [...current, id] : current);
  };
  const acceptOrContinue = async () => {
    if (!selectedRequest || busy) return;
    let detail = selectedRequest.raw;
    if (detail.available_actions?.accept) {
      const accepted = await perform("accept");
      if (!accepted) return;
      detail = accepted;
    }
    ...
    if (detail.available_actions?.start_preparing && !detail.available_actions?.reserve_units) {
      const preparing = await perform("start-preparing");
      if (!preparing) return;
      detail = preparing;
    }
    if (detail.available_actions?.reserve_units) {
      await openUnitsDialog();
    }
  };
  const reserveAndPrepare = async () => {
    ...
    if (ready.status === "ready") onReady?.();
  };

  return (
    <>
    {error && <p role="alert" className="mb-2 text-xs text-[#B4233A]">{error}</p>}
      ...
      {selectedRequest && (
        <div className="flex min-h-[392px] flex-col px-[15px] pb-[18px] pt-[14px]">
          ...
          <div className="mt-[18px] grid grid-cols-2 gap-[11px]">
            ...
            <article className="relative box-border flex h-[91px] grow flex-col items-start rounded-[18px] border border-[#ECEEF0] bg-white p-[12px] shadow-[0_3px_10px_rgba(30,36,50,0.024)]">
              <div><p className="text-[10px] text-[#98a3aa]">التغطية</p><p dir="ltr" className="mt-1 text-right font-sans text-[20px] font-bold leading-6 text-[#172a3a]">{selectedRequest.raw.units_reserved}/{selectedRequest.units}</p><p className="mt-1 text-[9px] text-[#42887f]">وحدة</p></div>
          ...
          <dl className="mt-[12px] divide-y divide-[#edf0f1] text-[10px]">
            <div className="grid grid-cols-3 py-[9px]"><div><dt className="font-bold text-[#243746]">الجهة الطالبة</dt><dd className="mt-0.5 text-[9px] text-[#9aa5aa]">{selectedRequest.hospital}</dd></div><div className="-translate-x-[12px] justify-self-center text-right font-['Tajawal']"><dt className="text-[10px] font-bold text-[#243746]">تاريخ الحاجة</dt><dd dir="ltr" className="mt-0.5 text-right text-[9px] text-[#9aa5aa]">{selectedRequest.date}</dd></div></div>
            <div className="py-[10px]"><dt className="font-bold text-[#243746]">سبب الطلب</dt><dd className="mt-0.5 text-[9px] text-[#9aa5aa]">{selectedRequest.raw.description || selectedRequest.raw.notes || "—"}</dd></div>
          </dl>
          ...
          {!completed && (
            <footer className="mt-auto flex gap-2 pt-4">
              {waitingForSending ? (
                <button type="button" disabled={busy || !selectedRequest.raw.available_actions?.complete} onClick={() => void perform("complete")} className="h-[33px] rounded-[8px] bg-[#9E1B32] px-[16px] text-[10px] font-bold text-white shadow-[0_3px_7px_rgba(158,27,50,0.16)] hover:bg-[#831529]">تم الإرسال</button>
              ) : (
                <>
                  <button type="button" disabled={busy || !selectedRequest.raw.available_actions || !(selectedRequest.raw.available_actions.accept || selectedRequest.raw.available_actions.reserve_units || selectedRequest.raw.available_actions.start_preparing || selectedRequest.raw.available_actions.mark_ready)} onClick={() => void acceptOrContinue()} className="h-[33px] rounded-[8px] bg-[#9E1B32] px-[16px] text-[10px] font-bold text-white shadow-[0_3px_7px_rgba(158,27,50,0.16)] hover:bg-[#831529]">قبول الطلب</button>
            ...
            {selectableUnits.map((unit) => (
              <label key={unit.id} className={`flex h-[36px] cursor-pointer items-center justify-between rounded-[11px] border px-[10px] text-[9px] transition ${selectedUnits.includes(unit.id) ? "border-[#d45c70] bg-[#fffafb]" : "border-[#e9edef] hover:border-[#d8e0e3]"}`}>
                <span className="flex items-center gap-[7px]">
                  ...
                  <strong dir="ltr" className="font-sans text-[9px] text-[#263A44]">{unit.unit_code}</strong>
                  <span dir="ltr" className="font-sans text-[8px] text-[#9aa5aa]">{unit.blood_type}</span>
                  <span className="rounded bg-[#e8f7f3] px-1 py-0.5 text-[8px] font-semibold text-[#248e73]">متاحة</span>
                </span>
                <span className="whitespace-nowrap font-['Tajawal'] text-[11px] font-normal text-[#7d8b96]">تنتهي <span dir="ltr" className="inline-block">{new Date(unit.expires_at).toLocaleDateString("en-GB")}</span></span>
              </label>
            ))}
          </div>
          {error && <p role="alert" className="mt-2 text-[10px] text-[#B4233A]">{error}</p>}
