    ...
    <section className="mt-5 grid grid-cols-1 gap-5 lg:grid-cols-2">
      <article className="h-fit w-full self-start rounded-[18px] bg-white px-[15px] pb-[20px] pt-[12px] shadow-[0_5px_20px_rgba(28,50,58,0.025)]">
        ...
        <div className="mt-[12px] grid grid-cols-3 gap-[7px]">
          {actions.map(({ label, description, icon: Icon, activeClass, iconClass }, index) => (
            <button
              key={label}
              type="button"
              onClick={() => setSelectedAction(index)}
              aria-pressed={selectedAction === index}
              className={`flex min-h-[100px] cursor-pointer flex-col items-center justify-center rounded-[11px] border border-[rgba(238,241,242,0.45)] transition-colors ${
                selectedAction === index ? activeClass : "bg-white"
              }`}
            >
