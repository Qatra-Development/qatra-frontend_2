...
export default function LatestRequests({ showHeader = true, toolbar, incomingMode = false, statusOverride }: LatestRequestsProps) {
  ...
  const waitingForSending = incomingMode && statusOverride === "بانتظار الإرسال";
  const completed = incomingMode && statusOverride === "مكتمل";
  const canOpenDetails = incomingMode && (statusOverride === "قيد الاستجابة" || waitingForSending || completed);
  ...
  return (
    ...
    <dialog
      ...
      className="m-auto w-[min(405px,100vw)] max-h-[100dvh] overflow-y-auto rounded-[12px] border-0 bg-white p-0 font-['IBM_Plex_Sans_Arabic'] text-[#263A44] shadow-[0_20px_60px_rgba(20,32,42,0.2)] backdrop:bg-[#1F2937]/55"
    >
      {selectedRequest && (
        <div className="flex min-h-[392px] flex-col px-[15px] pb-[18px] pt-[14px]">
          ...
          <dl className="mt-[12px] divide-y divide-[#edf0f1] text-[10px]">
            <div className="grid grid-cols-3 py-[9px]"><div><dt className="font-bold text-[#243746]">الجهة الموردة</dt><dd className="mt-0.5 text-[9px] text-[#9aa5aa]">مركز الدم الإقليمي 01</dd></div><div className="-translate-x-[20px] text-center font-['IBM_Plex_Sans_Arabic']"><dt className="text-[10px] font-bold text-[#243746]">تاريخ إنشاء الطلب</dt><dd className="mt-0.5 text-[9px] text-[#9aa5aa]">24/9/2026</dd></div></div>
          ...
          {!completed && (
            <footer className="mt-auto flex gap-2 pt-4">
              {waitingForSending ? (
                <button type="button" onClick={() => dialogRef.current?.close()} className="h-[33px] rounded-[8px] bg-[#9E1B32] px-[16px] text-[10px] font-bold text-white shadow-[0_3px_7px_rgba(158,27,50,0.16)] hover:bg-[#831529]">تم الإرسال</button>
              ) : (
                <>
    ...
    <dialog
      ...
      className="m-auto w-[min(433px,calc(100vw-14px))] max-h-[calc(100dvh-14px)] overflow-y-auto rounded-[13px] border-0 bg-white p-0 font-['IBM_Plex_Sans_Arabic'] text-[#263A44] shadow-[0_20px_60px_rgba(20,32,42,0.2)] backdrop:bg-[#1F2937]/55"
    >
      {selectedRequest && (
        <div className="flex min-h-[452px] flex-col px-[17px] pb-[18px] pt-[24px]">
          ...
          <p className="mt-[18px] whitespace-nowrap text-right font-['IBM_Plex_Sans_Arabic'] text-[9px] font-normal text-[#85939b]">حدد 3 وحدات متاحة من الفصيلة المطلوبة، الوحدة المحجوزة لن تكون متاحة لطلب آخر</p>
          <div className="mt-[12px] flex h-[36px] items-center justify-between rounded-[12px] border border-[#e9edef] bg-[#f7fafb] px-[10px]">
            <span className="font-['IBM_Plex_Sans_Arabic'] text-[9px] font-normal text-[#84929a]">اختر 3 وحدات للمتابعة</span>
            <span className="inline-flex items-center gap-[10px] font-['IBM_Plex_Sans_Arabic'] text-[15px] font-bold text-[#9E1B32]"><span dir="ltr">{selectedUnits.length} / 3</span><span className="text-[9px] font-normal text-[#84929a]">تم اختيارها</span></span>
          </div>
            ...
            {availableUnits.map((unit) => (
              <label key={unit.id} className={`flex h-[36px] cursor-pointer items-center justify-between rounded-[11px] border px-[10px] text-[9px] transition ${selectedUnits.includes(unit.id) ? "border-[#d45c70] bg-[#fffafb]" : "border-[#e9edef] hover:border-[#d8e0e3]"}`}>
                <span className="flex items-center gap-[7px]">
                  <input type="checkbox" checked={selectedUnits.includes(unit.id)} onChange={() => toggleUnit(unit.id)} disabled={!selectedUnits.includes(unit.id) && selectedUnits.length >= 3} className="sr-only" />
                ...
                <span className="whitespace-nowrap font-['IBM_Plex_Sans_Arabic'] text-[11px] font-normal text-[#7d8b96]">تنتهي <span dir="ltr" className="inline-block">{unit.expires}</span></span>
              </label>
            ))}
          </div>

          <footer className="mt-auto flex gap-[7px] pt-[14px]">
            <button type="button" disabled={selectedUnits.length !== 3} onClick={() => unitsDialogRef.current?.close()} className="h-[33px] rounded-[9px] bg-[#9E1B32] px-[20px] text-[10px] font-bold text-white disabled:cursor-not-allowed disabled:bg-[#dfc9ce]">تأكيد حجز الوحدات</button>
