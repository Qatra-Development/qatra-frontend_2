...
export default function LatestRequests({ showHeader = true, toolbar, incomingMode = false, statusOverride, onChanged, onCompleted, onReady }: LatestRequestsProps) {
  ...
  const openUnitsDialog = async (detail?: BloodRequest) => {
    const request = detail ?? selectedRequest?.raw;
    if (!request?.available_actions?.reserve_units) return;
    setSelectedUnits([]);
    try {
