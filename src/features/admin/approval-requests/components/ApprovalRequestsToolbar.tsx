...
import { SERVICE_SCOPE_OPTIONS } from "../config/approval-requests.config";

import type { ServiceScopeFilter } from "../types/approval-requests.types";

interface ApprovalRequestsToolbarProps {
  searchQuery: string;
...
export default function ApprovalRequestsToolbar({
  searchQuery,
  onSearchChange,

  serviceScope,
  onServiceScopeChange,
}: ApprovalRequestsToolbarProps) {
  return (
    <div
      dir="rtl"
      className="
    ...
    >
      ...
      <div className="relative w-full sm:w-[165px]">
        <select
          ...
          aria-label="فلترة حسب نطاق الخدمة"
          className="
            h-11 w-full
            cursor-pointer
            appearance-none
